...
function mobilePlayerCards(rows,info){
  return rows.length?rows.map(p=>'<article class="member-card" data-member-name="'+esc(p.player)+'"><header><div>'+playerCardLink(p.player,playerFaceChip(p.player,false,info?.year),info)+'<p>'+esc(teamDisplayName(p.displayTeam||p.mainTeam||''))+'</p></div><span class="member-label">출석 '+p.attendanceRate+'% · 🏅 '+(p.achievementCount||0)+'</span></header>'+careerFrameListHtml(p.frameState)+mobileMetrics([['등번호',esc(String(p.shirtNumber??'')||'—')],['포지션',esc(p.position||'—')],['팀경기',p.teamGames],['득점',p.g],['도움',p.a||0],['승점',p.pts],['출석',p.att],['선방',p.sv||0],['MOM',p.mom||0]])+'<div class="member-card-foot">'+p.w+'승 '+p.d+'무 '+p.l+'패 · 승률 '+p.winRate+'% · 개인파울 '+(p.f||0)+'</div>'+playerListAchievements(p.player,p.careerCutoff)+'</article>').join(''):'<p class="empty">선택 기간에 선수 기록이 없습니다.</p>';
}
